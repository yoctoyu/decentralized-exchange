...
```
$ git clone https://github.com/yoctoyu/decentralized-exchange.git
```
...
- Open Custom RPC
- Configure your private network by adding `http://localhost:7545` on the URL and `1377` as a chain ID.
...
```
$ truffle exec scripts/seed-exchange.js --network goerli
```
...
This project consists in a decentralized exchange to allow users making p2p trades between Ether and a ERC-20 token called YYU Token. It includes:

- A smart contract which contains an ERC-20 token called YYU Token
...
Here is where the user can deposit / withdraw Ether or YYU Token in the Exchange. It also reflects the balances, not only exchange balances but also wallet ballances,
...
The Price Chart reflects the price of the YYU Token via candlestick chart.
